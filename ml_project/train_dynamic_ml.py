...
def extract_features(frame):
    """
    Same 77-feature extractor as static model.
    """
    # Find first valid hand
    hand_data = None
...
def main():
    print("=== Signs Sense: DYNAMIC SHAPE TRAINER ===")
    movement_dir = r"c:/Users/USER/Desktop/DTW/templates/movement/"
    files = sorted(glob.glob(os.path.join(movement_dir, "**/*.json"), recursive=True))
    
    X, y = [], []
    for f in files:
        sign = os.path.basename(f).split('.')[0] # Keep exact sign name, e.g., 'TalkingtooMuch'
        with open(f, 'r') as jf:
            try:
    ...
    print(f"Training complete. Self-Accuracy: {acc:.2f}%")

    joblib.dump(clf, "dynamic_ml_model.pkl")
