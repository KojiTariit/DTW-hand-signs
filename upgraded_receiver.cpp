...
// Extracts exactly what m2cgen expects (39 features), PLUS the Face Distances
std::vector<float> extract_ml_features(const Frame& f) {
    ...
    // 6. Thumb-Cross Matrix (4 Features) - Solves A vs S
...
int main() {
    std::cout << "--- UPGRADED HYBRID RECEIVER (w/ FACE CONTEXT) ONLINE ---" << std::endl;
    ...
    char buffer[16384]; // Upgraded buffer for Face json
            ...
            try {
                ...
                if (data.contains("type") && data["type"] == "END_OF_SIGN") {
                    if (current_sign_buffer.size() >= 5) {
                        ...
                        std::cout << "\n[SIGN COMPLETE] Frames: " << current_sign_buffer.size() 
                                  << " | Wrist Move: " << max_wrist_dist << std::endl;
